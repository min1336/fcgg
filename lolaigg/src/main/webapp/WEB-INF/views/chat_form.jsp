...
<body>
    ...
    <script>
        ...
        chatForm.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/lolaigg/chat.do', {
                    method: 'POST',
                    headers: {
                ...
                loadingIndicator.style.display = 'none'; // 🌟 응답 오면 로딩 숨기기
                addMessageToWindow(botHtmlReply, 'bot'); // 🌟 봇 메시지 추가
                
            } catch (error) {
                console.error('Error fetching chat reply:', error);
                const errorHtml = marked.parse(`**오류가 발생했습니다:**\n\`\`\`\n${error.message}\n\`\`\``);
                
                loadingIndicator.style.display = 'none'; // 🌟 오류 시에도 로딩 숨기기
                addMessageToWindow(errorHtml, 'bot'); // 🌟 오류 메시지 추가
            } finally {
                // 🌟 finally에서는 스크롤만 처리
                chatWindow.scrollTop = chatWindow.scrollHeight;
            }
